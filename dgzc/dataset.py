...
class DGZCDataset(Dataset):
    ...
    def __getitem__(self, index: Any) -> Any:
        ...
        face_image : torch.Tensor = torch.as_tensor(face_image, dtype=torch.float32).permute(2, 0, 1).unsqueeze(0)
        face_image = F.interpolate(face_image, size=self.size, mode='bicubic')
        face_image = face_image.squeeze(dim=0).clamp(0, 255.)
